...
const TablaProductosAdmin = ({ tiendaId }) => { // Recibe tiendaId como prop
  ...
  const listarProductos = async () => {
    ...
    try {
      const url = `${import.meta.env.VITE_BACKEND_URL}/administrador/tienda/productos/${tiendaId}`;
  ...
  const eliminarProducto = async (idProducto) => {
    try {
      const url = `${import.meta.env.VITE_BACKEND_URL}/administrador/producto/${idProducto}`;
            ...
            <tbody>
              {productos.map((producto) => (
                <tr key={producto._id}>
                  ...
                  <td className="text-center py-2 px-4 border-b border-gray-900 text-white">{producto.Nombre_producto}</td> {/* Cambié nombre por Nombre_producto */}
                  <td className="text-center py-2 px-4 border-b border-gray-900 text-white">
                    ...
                    >
                      {producto.Estado ? "Disponible" : "No Disponible"} {/* Cambié estado por Estado */}
                    </span>
                  </td>
                  <td className="text-center py-2 px-4 border-b border-gray-900 text-white">
                    <button 
